...
EXPORT\_SCALE, EXPORT\_CRS = 10, 'EPSG:4326'
CLOUD\_PROB\_THRESHOLD = 40
WAIT\_TIME  = int(os.getenv('WAIT\_TIME', 3600))
FILE\_TIMEOUT = 600

# ---------- Secrets / env ----------

SA\_KEY   = os.getenv('SA\_KEY\_PATH', 'sa-key.json')
SFTP\_HOST= os.environ\['SFTP\_HOST']; SFTP\_PORT = int(os.environ\['SFTP\_PORT'])
SFTP\_USER= os.environ\['SFTP\_USER']; SFTP\_PASS = os.environ\['SFTP\_PASS']
SFTP\_DEST= os.environ\['SFTP\_DEST\_FOLDER']
SMTP\_SRV = os.environ\['SMTP\_SERVER']; SMTP\_PORT = int(os.environ\['SMTP\_PORT'])
SMTP\_USR = os.environ\['SMTP\_USER'];   SMTP\_PWD  = os.environ\['SMTP\_PASS']
EMAILS   = os.environ\['ALERT\_EMAILS'].split(',')

# ---------- Initialisation GEE ----------

cred = service\_account.Credentials.from\_service\_account\_file(SA\_KEY, scopes=\[
'[https://www.googleapis.com/auth/cloud-platform](https://www.googleapis.com/auth/cloud-platform)',
'[https://www.googleapis.com/auth/earthengine](https://www.googleapis.com/auth/earthengine)',
'[https://www.googleapis.com/auth/drive](https://www.googleapis.com/auth/drive)'])
ee.Initialize(cred, project=cred.project\_id)
drv = build('drive', 'v3', credentials=cred, cache\_discovery=False)
...
def \_retry(fun, \*a, \*\*k):
for i in range(1, RETRY\_MAX+1):
try: return fun(\*a, **k)
except (ssl.SSLError, socket.error, HttpError) as e:
if i == RETRY\_MAX: raise
time.sleep(BASE \* 2**(i-1) \* (0.5 + random.random()/2))

def drv\_list(svc, \*\*kw): return \_retry(lambda: svc.files().list(\*\*kw).execute())\['files']
def drv\_del(svc, fid):
try: \_retry(lambda: svc.files().delete(fileId=fid).execute())
except HttpError as e:
if e.resp.status not in (403, 404): raise

def drv\_download(svc, fid, path):
with open(path,'wb') as h:
req = svc.files().get\_media(fileId=fid)
dl = MediaIoBaseDownload(h, req)
done = False
while not done: \_, done = \_retry(dl.next\_chunk)

def sftp\_mkdirs(sftp, path):
cur = ''
...
all\_sent, all\_errs = \[], \[]
START\_DATE = ee.Date('2025-03-25')
END\_DATE   = ee.Date('2025-05-25')

for site\_id in SITE\_IDS:
AOI\_FC = ee.FeatureCollection(site\_id)
AOI = ee.Feature(AOI\_FC.first())
AOI\_GEOM = AOI.geometry()
SITE\_NAME = AOI.get('Nom').getInfo() or site\_id.split('/')\[-1]
...
body = \[f"{len(all\_sent)} fichiers transférés:", "- " + "\n- ".join(all\_sent)]
if all\_errs:
body.append(f"{len(all\_errs)} erreur(s):\n" + "\n".join(f"{n}: {e}" for n,e in all\_errs))
msg = MIMEMultipart(); msg\['From']=SMTP\_USR; msg\['To']=','.join(EMAILS)
msg\['Subject']="GEE Indices : " + ("Succès" if not all\_errs else "Succès (avec erreurs)")
msg.attach(MIMEText("\n\n".join(body),'plain'))
with smtplib.SMTP(SMTP\_SRV,SMTP\_PORT) as s:
s.starttls(); s.login(SMTP\_USR,SMTP\_PWD); s.send\_message(msg)
