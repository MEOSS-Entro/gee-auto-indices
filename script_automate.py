...
– attente dynamique des tâches EE
...
# temps d’attente : 4 h max ou 10 min × nb tâches, au plus grand des deux
WAIT_TIME        = int(os.getenv("WAIT_TIME", 14400))   # plafond global
PER_TASK_WAIT    = int(os.getenv("PER_TASK_WAIT", 600)) # 10 min / tâche
POLL_EVERY       = 30                                   # s
FILE_TIMEOUT     = 900                                  # s .tif Drive
...
CLOUD_PROB_THRESHOLD, EXPORT_SCALE, EXPORT_CRS = 40, 10, "EPSG:4326"
empty_img = ee.Image.constant([-32767]*len(INDICES)).rename(INDICES).updateMask(ee.Image.constant(0))

# ─────────── utilitaires Drive (retry) ──────────────────────────────────────
RETRY_MAX, BACKOFF_BASE = 6, 3
def _retry(fun,*a,**k):
    for i in range(1,RETRY_MAX+1):
        try: return fun(*a,**k)
        except (ssl.SSLError,socket.error,HttpError) as e:
            if i==RETRY_MAX: raise
            d = BACKOFF_BASE*2**(i-1)*(0.5+random.random()/2)
            print(f"Retry {i}/{RETRY_MAX} dans {d:.1f}s – {e}"); time.sleep(d)

def drv_list(svc,**kw): return _retry(lambda:svc.files().list(**kw).execute())["files"]
def drv_del (svc,fid):   _retry(lambda:svc.files().delete(fileId=fid).execute())
...
# ─────────── Fonctions EE (indices & compositing) ───────────────────────────
...
def dekad_composite(start,end,geom):
    ...
    joined = ee.Join.saveFirst("cloud_prob").apply(
               s2sr, s2prob, ee.Filter.equals(leftField="system:index", rightField="system:index"))
...
for site_id in SITE_IDS:
    ...
    # nettoyage
    for kid in drv_list(drv,q=f"'{folder_id}' in parents and trashed=false",fields="files(id)"):
        drv_del(drv,kid["id"])
    ...
    for off in range(0,n_days-period+1,step):
        ...
        for b in INDICES:
            fn = f"{site}_{b}_{date_str}"
            t = ee.batch.Export.image.toDrive(
                    image=img.select(b), description=fn, folder=site, fileNamePrefix=fn,
    ...
    while pending and time.time()-t0 < site_wait:
        ...
        for tid,t in list(pending.items()):
            st = t.status()["state"]
            if st in ("COMPLETED","FAILED","CANCELLED","CANCEL_REQUESTED"):
                done+=1; pending.pop(tid)
        pct = 100*done/len(tasks)
        print(f"{done}/{len(tasks)} tâches terminées ({pct:.0f} %)", end="\r")
        if pending: time.sleep(POLL_EVERY)
    print()  # sauter ligne

    if pending:
        raise RuntimeError(f"{site} : timeout des tâches EE ({len(pending)} restantes)")

    # Attente des .tif Drive (même nombre que tasks)
    want, t0 = len(tasks), time.time()
    while True:
        ...
        print(f"{len(ready)}/{want} .tif prêts...",end="\r"); time.sleep(15)
    print()
