...
# -*- coding: utf-8 -*-
"""
EE → Drive → SFTP  (service‑account, 100 % gratuit)
- vide TOUS les dossiers homonymes sur Drive puis recrée un dossier unique
- exports Sentinel‑2 → Drive, polling réel des tâches
- attend la présence effective des .tif (Drive finalise le binaire)
- transfert SFTP avec création récursive du dossier et retry réseau
- rapport e‑mail
"""
...
POLL_EVERY     = 30                                  # pour EE
FILE_TIMEOUT   = 600                                 # 10 min max pour .tif
...
# ---------- 2. Dossier Drive (suppression doublons) -------------------------
q   = (f"name='{DRIVE_FOLDER}' and mimeType='application/vnd.google-apps.folder' "
       "and trashed=false")
folds = drv_list(drv, q=q, fields='files(id)')
for f in folds:                     # supprime TOUT
    kids = drv_list(drv, q=f"'{f['id']}' in parents and trashed=false",
                    fields='files(id)')
    for k in kids: drv_del(drv,k['id'])
    drv_del(drv,f['id'])
    print(f"• Ancien dossier supprimé {f['id']}")
...
print("• Dossier recréé :", FOLDER_ID)
...
# ---------- 4. Polling tâches EE --------------------------------------------
pend = {t.status()['description']:t for t in tasks}
...
# ---------- 5. Attente apparition des .tif (Drive) --------------------------
want = len(tasks)
t0 = time.time()
while True:
    files = drv_list(drv,
    ...
    if time.time()-t0 > FILE_TIMEOUT:
        raise RuntimeError("Fichiers Drive incomplets après 10 min")
    print(f"⌛ {len(ready)}/{want} .tif prêts…")
    time.sleep(15)

# ---------- 6. SFTP ----------------------------------------------------------
def sftp_mkdirs(sftp, path):
    """crée récursivement path (unix) si inexistant"""
...
sftp_mkdirs(sftp, SFTP_DEST)          # assure l’existence du dossier
...
for f in ready:
    ...
    try:
        sftp.put(tmp, f"{SFTP_DEST.rstrip('/')}/{name}")
        sent.append(name)
...
msg = MIMEMultipart()
msg['From'] = SMTP_USR; msg['To'] = ','.join(EMAILS)
